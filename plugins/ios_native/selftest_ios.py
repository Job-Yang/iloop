...
def test_sim_build_command_and_success_marker() -> None:
    with tempfile.TemporaryDirectory() as d:
        ...
        check("build: 用了 xcodebuild", "xcodebuild" in argv)
        check("build: 模拟器 sdk 正确", "iphonesimulator" in argv)
...
def test_real_build_has_signing_and_provisioning() -> None:
    with tempfile.TemporaryDirectory() as d:
        ...
        argv = " ".join(fake.calls[0])
        check("real build: 带 -allowProvisioningUpdates", "-allowProvisioningUpdates" in argv)
        check("real build: 注入 DEVELOPMENT_TEAM", "DEVELOPMENT_TEAM=ABCDE12345" in argv)
        check("real build: destination 为真机", "generic/platform=iOS" in argv)
...
def test_sim_install_launch_screenshot_commands() -> None:
    with tempfile.TemporaryDirectory() as d:
        ...
        check("install: simctl install 命令正确", "simctl install booted /x/App.app" in install_argv)
        check("launch: simctl launch 命令正确", "simctl launch booted com.x.app" in launch_argv)
...
def run() -> int:
    for fn in [
        ...
        test_real_build_has_signing_and_provisioning,
