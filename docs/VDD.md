...
前面绕了一大圈的道理，最后收成十一条能直接拿去用的守则。每一条都是一把尺子，拿它去量一个具体的设计决定，合不合格一比就知道：
...
11. **每次改动收口前，都回头看一眼它对整体的影响。** 前十条验的是"我做的这件事做成没有"，这条验另一个方向——"这件事放进整体，该不该这么改"。一行代码也可能牵动全局，所以别拿 diff 大小判断要不要看，每次都看，只是深浅按代价走：小改瞥一眼骨架顺不顺就行；大改和删东西，得把"这东西原本服务谁、删了谁受影响"一条条查清、留下证据。单看这次改动通过不算完成，它放进整体也立得住，才算。
...
**第三，架构慢慢烂掉，现在多了一道刹车，但还刹不死。** 早先这套方法只验单次改动，管不住"几百次各自都通过、合起来烂成一团"——每个单看都对，合起来成了没人敢动的一团。现在加了第 11 条守则：每次改动收口前都要回头看它对整体的影响，把"谁站在更高处替整体架构负责"从一句空话，做成了一个每次都跑、按代价伸缩的最小动作。但它治的是"每次都记得抬头看一眼"，治不了"看了也判断错"——判断整体影响的终究还是同一颗脑子，长期累积的架构漂移还得靠定期人工复盘兜底。方向对了，别指望它一劳永逸。
